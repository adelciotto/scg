...
static void lissajous_get_point(lissajous_t lissajous, float32_t *out_x, float32_t *out_y, float32_t t) {
	// Reference: https://en.wikipedia.org/wiki/Parametric_equation#Lissajous_Curve
    *out_x = lissajous.scale_x * cosf(lissajous.a * t);
    *out_y = lissajous.scale_y * sinf(lissajous.b * t);
}

static void init_metablobs(metablobs_t *metablobs, float32_t const_a, float32_t const_b) {
	blob_t *blobs = metablobs->blobs;

	blobs[0].lissajous = (lissajous_t){
		.scale_x = 60.0f,
		.scale_y = 60.0f,
		.a = 3.0f,
		.b = 2.0f
...
static void update_metablobs(metablobs_t *metablobs, float32_t t) {
	for (int i = 0; i < NUM_BLOBS; i++) {
		...
		lissajous_get_point(blob->lissajous, &blob->x, &blob->y, t * blob->speed_modifier);
	}
}
