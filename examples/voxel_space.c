...
static scg_return_status_t init(terrain_t *terrain, camera_t *camera) {
    scg_image_t color_map;
    scg_return_status_t return_status =
        scg_image_create_from_bmp(&color_map, "assets/color_map.bmp");
    ...
    scg_image_t height_map;
    return_status =
        scg_image_create_from_bmp(&height_map, "assets/height_map.bmp");
    if (return_status.is_error == SCG_TRUE) {
        return return_status;
    }
    ...
    for (int y = 0; y < map_h; y++) {
        for (int x = 0; x < map_w; x++) {
            ...
            scg_pixel_t pixel = height_map.pixels[i];
            pixel.packed = pixel.color.r;

            height_map.pixels[i] = pixel;
        }
    }
...
static scg_pixel_t shade_pixel(scg_pixel_t src, float32_t t) {
    float32_t r = (float32_t)src.color.r;
    float32_t g = (float32_t)src.color.g;
    float32_t b = (float32_t)src.color.b;

    r += t * (float32_t)(255.0f - r);
    g += t * (float32_t)(255.0f - g);
    b += t * (float32_t)(255.0f - b);

    return scg_pixel_create((uint8_t)r, (uint8_t)g, (uint8_t)b);
}
...
static void draw(scg_screen_t *screen, terrain_t terrain, camera_t camera) {
    ...
    for (float32_t z = 1; z < camera.max_distance; z += dt) {
        ...
        for (int x = 0; x < screen_w; x++) {
            ...
            scg_pixel_t color_map_pixel = terrain.color_map.pixels[i];
            scg_pixel_t height_map_pixel = terrain.height_map.pixels[i];
            ...
            float32_t height_on_screen =
                ...
                camera.horizon;
            scg_screen_draw_vertical_line_fast(screen, x, height_on_screen,
                                               ybuffer[x], color_map_pixel);
...
int main(void) {
    ...
    terrain_t terrain;
    camera_t camera;
    return_status = init(&terrain, &camera);
    if (return_status.is_error == SCG_TRUE) {
    ...
    while (scg_screen_is_running(&screen)) {
        ...
        int map_height = terrain.height_map.pixels[map_offset].packed + 10;
